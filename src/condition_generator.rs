...
use crate::{
    ...
    condition_utils::{
        add_read_only_input_file_hashes, dir_created_by_exec, preconditions_contain_stat_fact,
        update_file_posts_with_renamed_dirs, FileType,
    },
...
pub fn generate_preconditions(
    aggregated_hashes: HashMap<PathBuf, Vec<u8>>,
    events: ExecSyscallEvents,
) -> Preconditions {
    ...
    // Also add missing input file hashes.
    let updated_file_preconds = add_read_only_input_file_hashes(file_preconds, aggregated_hashes);
    Preconditions::new(dir_preconds, updated_file_preconds)
}
...
mod tests {
    ...
    fn test_failed_access_then_create() {
        ...
        let preconditions = generate_preconditions(HashMap::new(), exec_syscall_events.clone());
